...
$.fn.checkUserId = function() { // 사용자 ID 중복 체크 플러그인
	var self = this;
	var idInput = this.closest('.form-group').find('input'); // 사용자 ID 입력
																// input
	...
	self.click(function() {
		...
		if (!userId)
			return alert('사용자ID를 입력하세요');

		$.get('id-check/' + userId, function(data) {
			if (data.result == 'ok') {
				msgSpan.html(data.message).removeClass('error'); //css class에 대한 이름이다.
				submitBtn.prop('disabled', false); // submit버튼 활성화
				self.prop('disabled', true); // ID 체크버튼 비활성화
			} else {
